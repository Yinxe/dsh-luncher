...
- **Web 界面打开方式（Profile 页「打开方式」）**：默认在应用内**独立窗口**打开 DSH 界面——没有浏览器地址栏，像桌面版一样直接用；一个地址只有一个窗口（再次点击是把已有窗口聚焦前置），多个实例同时运行时可以各开一个窗口并排使用。想回到浏览器打开，在 Profile 页把「打开方式」切到**系统默认浏览器**即可，该设置对顶栏「打开 DSH 界面」、首页、实例列表与实例终端的所有入口全局生效。
