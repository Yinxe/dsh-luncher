...
import { R2_MANIFEST_URL, REPO_URL, type ReleaseState } from "@/lib/release";

/**
 * 两条源的分工 —— 「可选 GitHub 或 R2」这件事本身需要解释清楚，
 * 因为它们的差别不只是「快慢」，而是**地址的生命周期**不同：
 * R2 的地址与版本无关（一条链接永远指向最新），GitHub 的地址与版本绑定（适合固定版本归档）。
 */
export function SourceCompare({ state }: { state: ReleaseState }) {
  ...
  const rows: { dim: string; r2: React.ReactNode; gh: React.ReactNode }[] = [
    ...
    },
    {
      ...
      r2: (
        <span className="flex items-start gap-2">
          ...
          <span>实测可用：不走代理拉 81 MB 的 AppImage，约 3.3 MB/s</span>
        </span>
      ),
      gh: (
        <span className="flex items-start gap-2">
    ...
    {
      dim: "下载到的文件名",
      r2: "对象键是平台名，靠响应头还原原始资产名",
      gh: "就是原始资产名，另附 .sig 签名文件",
    },
    {
      dim: "托管范围",
      r2: "6 个平台键：Windows .exe/.msi、Linux AppImage/.deb/.rpm、macOS 更新包",
      gh: "全部产物：以上所有 + macOS .dmg + 每个包的 .sig + latest.json",
    },
    {
      dim: "客户端里的角色",
      r2: (
    ...
    {
      dim: "适合谁",
      r2: "写进脚本、贴给别人、页面上的「一键下载」—— 不用关心版本号变没变",
    ...
    <section className="mx-auto max-w-[1180px] px-5 py-20 sm:px-8 sm:py-24">
      <Reveal>
        <SectionHeading
          ...
          lead={
            <>
              两条源上的安装包字节完全相同，客户端一律先验签再安装 ——{" "}
              <strong className="text-foreground">所以走镜像或代理也无法投毒</strong>
              ，改一个字节就验签失败。区别只在地址的生命周期与适用场景。
            </>
          }
        />
      </Reveal>
      ...
      <Reveal delay={80} className="mt-10">
        ...
        <div className="panel divide-y divide-hairline overflow-hidden lg:hidden">
          {rows.map((row) => (
          ...
          <Table>
            <TableHeader>
              <TableRow className="hover:bg-transparent">
                <TableHead className="w-[96px] text-[12px] whitespace-normal">维度</TableHead>
                <TableHead className="text-[12px] whitespace-normal">
                  <span className="flex items-center gap-2">
                    <Led tone="signal" />
                    自建源 · Cloudflare R2
                  </span>
            ...
            <TableBody>
              {rows.map((row) => (
                <TableRow key={row.dim} className="align-top">
                  <TableCell className="eyebrow pt-4 whitespace-normal text-foreground/70">{row.dim}</TableCell>
                  <TableCell className="pt-4 text-[13px] whitespace-normal text-muted-foreground">{row.r2}</TableCell>
                  <TableCell className="pt-4 text-[13px] whitespace-normal text-muted-foreground">{row.gh}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
      ...
      {/* 三条补充说明：不用卡片，靠发丝分隔线与间距分组 */}
      <Reveal delay={120} className="mt-6 grid gap-x-12 gap-y-8 border-t border-hairline pt-8 lg:grid-cols-3">
        <div>
          <h3 className="eyebrow text-primary">客户端读什么</h3>
          <p className="mt-2.5 max-w-[46ch] text-[13px] text-muted-foreground">
            Tauri 更新器格式的 <code className="num text-[12px]">latest.json</code>：版本号、发布日期、
            各平台地址与签名。自建源上这份清单必须随每次发布覆盖，否则客户端会一直以为没有新版。
          </p>
        ...
        <div>
          <h3 className="eyebrow text-primary">为什么地址带 ?v=</h3>
          <p className="mt-2.5 max-w-[46ch] text-[13px] text-muted-foreground">
            安装包对象用长缓存（<code className="num text-[12px]">immutable</code>），
            路径又永远叫 <code className="num text-[12px]">latest/…</code>：万一 CDN 边缘留着旧包，
            客户端就会下到与签名对不上的文件。所以每次发布换一个查询串当指纹 ——
            对人来说地址没变，对 CDN 来说是「每个版本一个新对象」。去掉{" "}
            <code className="num text-[12px]">?v=</code> 也能下，只是可能命中旧缓存。
          </p>
        </div>

        <div>
          <h3 className="eyebrow text-primary">在哪里切换</h3>
          <p className="mt-2.5 max-w-[46ch] text-[13px] text-muted-foreground">
            客户端：<strong className="text-foreground">设置 → 更新源</strong>，默认自建源；
            自建源清单陈旧或不可用时切 GitHub 逃生。
            <br />
            本页：顶部切换器，或用参数深链{" "}
            <a
              href="?source=github"
